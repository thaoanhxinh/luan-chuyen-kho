...
const PrintPhieuForm = ({ phieuId, onSuccess, onCancel }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await nhapKhoService.printPhieu(phieuId, formData);
    ...
    <div className="p-6">
      ...
      <div className="mb-4 p-3 bg-yellow-50 border border-yellow-200 rounded-lg">
        <p className="text-sm text-yellow-800">
          ...
          in. Nếu để trống, phiếu sẽ chỉ hiển thị tiêu đề mục để ghi tay sau
          này.
        </p>
      </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            ...
            <input
              ...
              value={formData.phuong_thuc_van_chuyen}
              onChange={handleInputChange}
              className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-green-500"
              placeholder="Ví dụ: Xe tải, Đường hàng không, Đơn vị tự vận chuyển..."
            />
          </div>
          {/* Tiểu đội trưởng kho */}
          <div>
